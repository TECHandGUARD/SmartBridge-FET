...
import { FileText, Download, Lock, ArrowLeft, Star, BookOpen, File, Bookmark, Loader2 } from 'lucide-react';
    ...
    try {
      ...
      if (user?.email) {
        ...
        if (!bookmarkError && bookmarkData) {
          setBookmarked(new Set(bookmarkData.map(b => b.resource_id)));
        }
      }
    } catch (err) {
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {error && (
          <div className="bg-red-50 border border-red-200 rounded-xl p-4 mb-4 text-sm text-red-700">
            {error}
            <Button variant="outline" size="sm" className="ml-4" onClick={loadResources}>Retry</Button>
          </div>
        )}
                ...
                <div key={resource.id} className="bg-card border border-border rounded-xl p-4 flex items-center gap-4 hover:shadow-sm transition-shadow">
                  ...
                  <div className="flex items-center gap-1.5 flex-shrink-0">
                    <Button 
                      size="icon" 
                      variant="ghost" 
                      className={`h-8 w-8 ${bookmarked.has(resource.id) ? 'text-primary' : 'text-muted-foreground hover:text-primary'}`} 
