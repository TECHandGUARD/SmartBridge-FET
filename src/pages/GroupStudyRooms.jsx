...
import { Users, MessageCircle, Send, StickyNote, Loader2 } from 'lucide-react';
import { SUBJECTS } from '@/lib/subjects';
import { toast } from 'sonner';
import StudyRoomBoard from '@/components/study/StudyRoomBoard';
...
export default function GroupStudyRooms() {
  ...
  }, [roomId, user?.email, loadMessages]);
        ...
        <div className="grid lg:grid-cols-4 gap-6">
          ...
          <div className="lg:col-span-3">
            <Card className="border-border flex flex-col" style={{ height: '600px' }}>
              {/* Room header */}
              <CardHeader className="pb-0 flex-shrink-0 border-b border-border">
                ...
                <div className="flex gap-1">
                  <button
                    ...
                    className={`flex items-center gap-1.5 px-4 py-2 text-sm font-medium border-b-2 transition-colors ${activeTab === 'chat' ? 'border-primary text-primary' : 'border-transparent text-muted-foreground hover:text-foreground'}`}
                  >
                    <MessageCircle className="w-4 h-4" /> Live Chat
                  </button>
                  <button
                    onClick={() => setActiveTab('board')}
                    className={`flex items-center gap-1.5 px-4 py-2 text-sm font-medium border-b-2 transition-colors ${activeTab === 'board' ? 'border-primary text-primary' : 'border-transparent text-muted-foreground hover:text-foreground'}`}
                  >
                    <StickyNote className="w-4 h-4" /> Message Board
                  </button>
                </div>
              </CardHeader>
              ...
              {/* Message Board Tab */}
              {activeTab === 'board' && (
                <StudyRoomBoard room={activeRoom} user={user} />
              )}
            </Card>
          </div>
        </div>
