...
// ✅ Admin emails – bypass this screen entirely
...
export default function TutorPendingScreen({ user, onRefresh }) {
  ...
  useEffect(() => {
    const checkAdmin = async () => {
      ...
      if (ADMIN_EMAILS.includes(user.email)) {
        console.log('✅ Admin detected in TutorPendingScreen – bypassing');
        setIsAdmin(true);
        setIsVerified(true);
      ...
      if (!profileError && profile) {
        if (profile.role === 'admin' || profile.is_super_admin === true) {
          console.log('✅ Admin role detected in TutorPendingScreen – bypassing');
          setIsAdmin(true);
          setIsVerified(true);
          setTimeout(() => {
            if (onRefresh) onRefresh();
  ...
  // Polling logic only for non-admin users
  ...
  }, [checking, isAdmin, user?.email, onRefresh]);

  // Poll every 30s only for non-admin users
  useEffect(() => {
    if (isAdmin) return;
  ...
  // ✅ If admin, show loading while redirecting (never render pending screen)
  if (isAdmin) {
    return (
